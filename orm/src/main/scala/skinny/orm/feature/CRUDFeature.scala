...
trait CRUDFeatureWithId[Id, Entity]
  ...
  override def createWithNamedValues(namedValues: (SQLSyntax, Any)*)(implicit s: DBSession = autoSession): Id = {
    ...
    val (insertColumns, insertValues) = allNamedValues.unzip
    if (useAutoIncrementPrimaryKey && !useExternalIdGenerator) {
      val id = withSQL { insert.into(this).columns(insertColumns: _*).values(insertValues: _*) }.updateAndReturnGeneratedKey.apply()
      ...
      withSQL { insert.into(this).columns(insertColumns: _*).values(insertValues: _*) }.update.apply()
